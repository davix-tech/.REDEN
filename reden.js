...
───────────────────────────────────────────── */
// Explicitly serve /sdk.js to ensure exact content-type and easier framework debugging
app.get("/sdk.js", (req, res) => {
  res.type("application/javascript");
  res.sendFile(path.join(__dirname, "public", "sdk.js"));
});

// Fallback static handling for any general public files
    ...
    if (normalizedEvent === "CHECKOUT_STARTED" && payload?.email) {
      await db.query(
        `INSERT INTO recovery_queue(site_id, session_id, customer_email, cart_data, incentive)
         VALUES ($1,$2,$3,$4,$5)
         ON CONFLICT (session_id, customer_email) DO NOTHING`,
...
// Excluded onboarding from public routes to guarantee explicit security segregation
const PUBLIC_ROUTES = ["/", "/health", "/sdk.js"];
const isPublic = (p) =>
  PUBLIC_ROUTES.includes(p) || /\.(png|jpg|jpeg|svg|css|js)$/i.test(p);
...
───────────────────────────────────────────── */
// Placed after CORS/Rate limits but BEFORE Auth to allow global access to client-side files
app.use(express.static(path.join(__dirname, "public")));
...
cron.schedule("*/15 * * * *", async () => {
  ...
  try {
    ...
    // FOR UPDATE SKIP LOCKED guarantees that multi-instance deployments won't pick up the same row
    const batch = await client.query(`
      UPDATE recovery_queue
      SET status='PROCESSING'
      WHERE id IN (
...
cron.schedule(
  "0 8 * * *",
  async () => {
    // Distributed Lock Check for multi-instance deployments
    if (redis) {
      const lockKey = `lock:cron:daily_report:${new Date().toISOString().split('T')[0]}`;
      // Acquire exclusive lock for 1 hour (3600000ms)
      const acquired = await redis.set(lockKey, "locked", "NX", "PX", 3600000);
      if (!acquired) {
        console.log("[DAILY REPORT CRON] Lock already held by another instance. Skipping.");
        return;
