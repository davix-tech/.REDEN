...
try {
await db.query(  CREATE TABLE IF NOT EXISTS sites (   id SERIAL PRIMARY KEY,   site_id VARCHAR(50) UNIQUE NOT NULL,   api_key VARCHAR(100) UNIQUE NOT NULL,   name VARCHAR(100) NOT NULL,   owner_email VARCHAR(255),   active BOOLEAN DEFAULT true,   created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP   );  );
...
const app = express();
// CRITICAL FIX 4: Reverting to 'true' for maximum deployment flexibility (Cloudflare/Nginx/etc)
...
// CRITICAL FIX 3: Expanded CORS headers for SDK evolution
res.setHeader(
...
if (!input || !secret || input.length !== secret.length) return false;
...
try {  
  ...
  if (e.code === "23505") return res.json(success({ duplicate: true, event_tracked: safeEventId }));  
  throw e;  
}  
...
if (normalizedEvent === "CHECKOUT_STARTED" && payload?.email) {  
  await db.query(  
    `INSERT INTO recovery_queue(site_id, session_id, customer_email, cart_data, incentive)  
     VALUES ($1,$2,$3,$4,$5) ON CONFLICT (session_id, customer_email) DO NOTHING`,  
    [req.site.site_id, session_id, payload.email, safeCart, "10% OFF"]  
  );  
}  

res.json(success({ event_id: safeEventId }));
...
/**  
 * CRITICAL FIX 2: Ensuring payload is at least an empty object   
 * to prevent downstream failures in pickAction().  
 */  
...
/**  
 * CRITICAL FIX 1: Matching the internal runtime's incentive labels.  
 * Original labels (LOW, MED, HIGH) were mismatching (INCENTIVE_LOW, etc).  
 */  
const discountMap = {   
  NONE: 0,   
  INCENTIVE_LOW: 5,   
...
try {
const r = await db.query(
SELECT COUNT(*) total, COUNT(*) FILTER (WHERE converted=true) conversions, COALESCE(SUM(revenue),0) revenue   FROM decisions WHERE site_id=$1 AND state='COMPLETED',
[req.site.site_id]
);
const row = r.rows[0];
res.json(success({
...
const batch = await client.query(  UPDATE recovery_queue   SET status = 'PROCESSING'   WHERE id IN (   SELECT id FROM recovery_queue   WHERE status = 'PENDING' AND created_at >= NOW() - INTERVAL '7 days'   FOR UPDATE SKIP LOCKED LIMIT 20   ) RETURNING *  );
claimed = batch.rows;
...
SELECT COUNT(*) total, COUNT(*) FILTER (WHERE converted=true) conversions, COALESCE(SUM(revenue),0) revenue   FROM decisions WHERE site_id=$1 AND state='COMPLETED' AND completed_at >= NOW() - INTERVAL '1 day',
[m.site_id]
);
...
/* ─────────────────────────────────────────────
ERROR ROUTING & GRACEFUL SHUTDOWN
───────────────────────────────────────────── */
