...
db.run(CREATE TABLE IF NOT EXISTS decisions (   id INTEGER PRIMARY KEY AUTOINCREMENT,   session_id TEXT,   cart_id TEXT,   action TEXT,   state TEXT DEFAULT 'EVALUATED',   created_at DATETIME DEFAULT CURRENT_TIMESTAMP   ));

db.run(CREATE TABLE IF NOT EXISTS outcomes (   id INTEGER PRIMARY KEY AUTOINCREMENT,   decision_id INTEGER,   converted BOOLEAN,   final_revenue REAL,   created_at DATETIME DEFAULT CURRENT_TIMESTAMP   ));
});
...
db.run(
INSERT INTO decisions (session_id, cart_id, action)   VALUES (?, ?, ?),
...
db.run(
UPDATE decisions   SET state='ACTIONED'   WHERE id=? AND state='EVALUATED',
[decision_id],
function () {
if (this.changes === 0) {
...
db.run(
INSERT INTO outcomes (decision_id, converted, final_revenue)   VALUES (?, ?, ?),
